...
\title{L'esprit statistique (original: Think Stats)}
\author{Allen B. Downey}

\newcommand{\thetitle}{L'esprit statistique (original: Think Stats)}
\newcommand{\thesubtitle}{Analyse exploratoire des données en Python}
...
Ce livre est une
introduction aux outils pratiques de l'analyse exploratoire des données.
L'organisation du livre suit le processus que j'utilise
lorsque je commence à travailler avec un ensemble de données :

\begin{itemize}

\item L'importation et le nettoyage: Quel que soit le format des données, il faut généralement du temps et des efforts pour les lire, les nettoyer et les transformer, et vérifier que tout est intact au cours du processus d'importation.
\index{cleaning}

\item Explorations à variable unique : Je commence généralement par examiner une variable à la fois, en découvrant la signification des variables, en examinant les distributions des valeurs et en choisissant des statistiques sommaires appropriées.
\index{distribution}

\item Des explorations par paires : Pour identifier les relations possibles entre les variables, je regarde des tableaux et des diagrammes de dispersion, et je calcule des corrélations et des ajustements linéaires.
\index{correlation}
\index{linear fit}

\item Analyse multivariée : S'il existe des relations apparentes entre les variables, j'utilise la régression multiple pour ajouter des variables de contrôle et étudier des relations plus complexes.
\index{multiple regression}
\index{control variable}

\item Estimation et vérification des hypothèses : Lorsque vous présentez des résultats statistiques, il est important de répondre à trois questions : Quelle est l'ampleur de l'effet ?  Quel est le degré de variabilité auquel nous devons nous attendre si nous effectuons à nouveau la même mesure ?  Est-il possible que l'effet apparent soit dû au hasard ?
\index{estimation}
\index{hypothesis testing}

\item Visualisation : Au cours de l'exploration, la visualisation est un outil important pour trouver des relations et des effets possibles.  Enfin, si un effet apparent résiste à l'examen, la visualisation est un moyen efficace de communiquer les résultats.
\index{visualization}

\end{itemize}

Ce livre adopte une approche informatique, qui présente plusieurs avantages par rapport aux approches mathématiques :
\index{computational methods}

\begin{itemize}

\item Je présente la plupart des idées en utilisant le code Python, plutôt que la notation mathématique.  En général, le code Python est plus lisible ; de plus, comme il est exécutable, les lecteurs peuvent le télécharger, l'exécuter et le modifier.

\item Chaque chapitre comprend des exercices que les lecteurs peuvent faire pour développer et solidifier leur apprentissage.  Lorsque vous écrivez des programmes, vous exprimez votre compréhension en code ; pendant que vous déboguez le programme, vous corrigez également votre compréhension.
\index{debugging}

\item Certains exercices impliquent des expériences pour tester le comportement statistique.  Par exemple, vous pouvez explorer le Théorème de la limite centrale (CLT) en générant des échantillons aléatoires et en calculant leurs sommes.  Les visualisations qui en résultent montrent pourquoi le CLT fonctionne et quand il ne fonctionne pas.
\index{Central Limit Theorem}
\index{CLT}

\item Certaines idées qui sont difficiles à saisir mathématiquement sont faciles à comprendre par simulation.  Par exemple, nous approximons les p-valeurs en effectuant des simulations aléatoires, ce qui renforce la signification de la p-valeur.
\index{p-value}

\item Comme le livre est basé sur un langage de programmation universel (Python), les lecteurs peuvent importer des données provenant de presque toutes les sources. 
 Ils ne sont pas limités aux ensembles de données qui ont été nettoyés et formatés pour un outil statistique particulier.

\end{itemize}

Le livre se prête à une approche par projet.  Dans ma classe, les étudiants travaillent sur un projet d'un semestre qui les oblige à poser une question statistique, à trouver un ensemble de données permettant d'y répondre et à appliquer chacune des techniques qu'ils apprennent à leurs propres données.

Pour illustrer mon approche de l'analyse statistique, le livre présente une étude de cas qui traverse tous les chapitres.  Il utilise des données provenant de deux sources :

\begin{itemize}

\item L'enquête nationale sur la croissance des familles (NSFG), menée par les centres américains de contrôle et de prévention des maladies (CDC) pour recueillir des informations sur la vie familiale, le mariage et le divorce, la grossesse, l'infertilité, l'utilisation de la contraception et la santé des hommes et des femmes.
...
\item Le Behavioral Risk Factor Surveillance System (BRFSS), mené par le National Center for Chronic Disease Prevention and Health Promotion pour suivre les conditions sanitaires et les comportements à risque aux États-Unis. ("track health conditions and risk behaviors in
  the United States")  (See \url{http://cdc.gov/BRFSS/}.)

\end{itemize}

D'autres exemples utilisent les données de l'IRS, du recensement américain et Boston Marathon.

Cette deuxième édition de {\it L'esprit statistique\/} comprend les chapitres de la première édition, dont beaucoup ont été considérablement révisés, et de nouveaux chapitres sur la régression, l'analyse des séries chronologiques, l'analyse de survie,
et des méthodes analytiques.  L'édition précédente n'utilisait pas les pandas, SciPy, ou StatsModels, donc tout ce matériel est nouveau.


\section{Comment j'ai écrit ce livre}

Lorsque les gens écrivent un nouveau manuel, ils commencent généralement par lire une pile de vieux manuels.  Ainsi, la plupart des livres contiennent le même matériel dans un ordre à peu près identique.

Ce n'est pas ce que j'ai fait.  En fait, je n'ai pratiquement pas utilisé de matériel imprimé pendant que j'écrivais ce livre, pour plusieurs raisons :

\begin{itemize}

\item Mon but était d'explorer une nouvelle approche de ce matériau, je ne voulais donc pas être trop exposé aux approches existantes.

\item Comme je mets ce livre à disposition sous une licence libre, je voulais m'assurer qu'aucune partie de ce livre n'était grevée par des restrictions de copyright.

\item De nombreux lecteurs de mes livres n'ont pas accès à des bibliothèques de documents imprimés, j'ai donc essayé de faire référence à des ressources qui sont disponibles gratuitement sur Internet.

\item Certains partisans des anciens médias pensent que l'utilisation exclusive des ressources électroniques est paresseuse et peu fiable.  Ils ont peut-être raison pour la première partie, mais je pense qu'ils ont tort pour la seconde, c'est pourquoi je voulais tester ma théorie.

% http://www.ala.org/ala/mgrps/rts/nmrt/news/footnotes/may2010/in_defense_of_wikipedia_bonnett.cfm

\end{itemize}

La ressource que j'ai utilisée plus que toute autre est Wikipédia. En général, le Les articles que j'ai lus sur des sujets statistiques étaient très bons (bien que j'aie fait quelques petits changements en cours de route).  J'inclus des références aux pages de Wikipedia tout au long du livre et je vous encourage à suivre ces liens ; dans de nombreux cas, la page Wikipedia complète là où ma description s'arrête.  Le vocabulaire et la notation dans ce livre sont généralement conforme à Wikipédia, à moins que j'aie une bonne raison de m'en écarter.  D'autres ressources que j'ai trouvées utiles sont les forum de statistiques sur Wolfram MathWorld et  Reddit. \url{http://www.reddit.com/r/statistics}.


\section{L'utilisation du code}
\label{code}

Le code et les données utilisés dans ce livre sont disponibles à l'adresse suivante
\url{https://github.com/AllenDowney/ThinkStats2}.  Git est un système de contrôle de version qui vous permet de garder une trace des fichiers qui composent un projet.  Une collection de fichiers sous le contrôle de Git est appelée un {\bf dépôt}.  GitHub est un service d'hébergement qui fournit de l'espace pour créer des dépôts Git et offre une interface web intuitive.
\index{repository}
\index{Git}
\index{GitHub}

La page d'accueil GitHub de mon dépôt propose plusieurs façons de travailler avec le code :

\begin{itemize}

\item Vous pouvez créer une copie de mon dépôt sur GitHub en appuyant sur le bouton {\sf Fork}.  Si vous n'avez pas encore de compte GitHub, vous devrez en créer un.  Après la fork , vous aurez votre propre dépôt sur GitHub que vous pourrez utiliser pour garder une trace du code que vous écrivez en travaillant sur ce livre.  Vous pourrez ensuite cloner le dépôt, ce qui signifie que vous ferez une copie des fichiers sur votre ordinateur.
\index{fork}

\item Oubien vous pourriez cloner mon dépôt.  Vous n'avez pas besoin d'un compte GitHub pour le faire, mais vous ne pourrez pas réécrire vos modifications sur GitHub.
\index{clone}

\item Si vous ne voulez pas du tout utiliser Git, vous pouvez télécharger les fichiers dans un fichier Zip en utilisant le bouton dans le coin inférieur droit de la page GitHub.

\end{itemize}

Tout le code est écrit pour fonctionner en Python 2 et Python 3 sans traduction.

J'ai développé ce livre en utilisant l'Anaconda de Continuum Analytics, qui est une distribution Python gratuite qui comprend tous les packages dont vous aurez besoin pour exécuter le code (et bien plus encore). J'ai trouvé qu'Anaconda était facile à installer.  Par défaut, l'installation se fait dans les repertoires personnels de l'utilisateur et non au niveau du système, de sorte que vous n'avez pas besoin de des privilèges.  Et il supporte à la fois Python 2 et Python 3.  Vous pouvez télécharger Anaconda sur \url{http://continuum.io/downloads}.
\index{Anaconda}

Si vous ne souhaitez pas utiliser l'Anaconda, vous aurez besoin des packages suivants :

\begin{itemize}

\item pandas pour la représentation et l'analyse des données,
  \url{http://pandas.pydata.org/};
\index{pandas}

\item NumPy pour le calcul numérique de base, \url{http://www.numpy.org/};
\index{NumPy}

\item SciPy pour le calcul scientifique, y compris les statistiques,
  \url{http://www.scipy.org/};
\index{SciPy}

\item StatsModels pour la régression et autres analyses statistiques,
\url{http://statsmodels.sourceforge.net/}; and
\index{StatsModels}

\item matplotlib pour la visualisation, \url{http://matplotlib.org/}.
\index{matplotlib}

\end{itemize}

Bien qu'il s'agisse de packages couramment utilisés, ils ne sont pas inclus dans toutes les installations Python, et ils peuvent être difficiles à installer dans certains environnements.  Si vous avez des difficultés à les installer, je vous recommande fortement d'utiliser Anaconda ou l'une des autres distributions Python qui incluent ces packages.
\index{installation}

Après avoir cloné le dépôt ou dézippé le fichier zip, vous devriez avoir
un dossier appelé {\tt ThinkStats2/code} avec un fichier appelé {\tt nsfg.py}.
Si vous exécutez {\tt nsfg.py}, il devrait lire un fichier de données, effectuer quelques tests et imprimer un
un message du genre "Tous les tests ont été réussis".  Si vous obtenez des erreurs d'importation, il signifie probablement qu'il y a des paquets que vous devez installer.

La plupart des exercices utilisent des scripts Python, mais certains utilisent également  IPython notebook.  Si vous n'avez jamais utilisé de notebook IPython auparavant, je vous suggère vous commencez par la documentation à 
\url{http://ipython.org/ipython-doc/stable/notebook/notebook.html}.
\index{IPython}

J'ai écrit ce livre en supposant que le lecteur est familier avec le noyau Python, y compris les fonctionnalités orientées objet, mais pas pandas, NumPy, et SciPy.  Si vous êtes déjà familier avec ces modules, vous pouvez sauter quelques sections.

Je suppose que le lecteur connaît les mathématiques de base, notamment les logarithmes, par exemple, et les sommations.  Je me réfère aux concepts de calcul à quelques endroits, mais vous n'avez pas à faire de calculs.

Si vous n'avez jamais étudié les statistiques, je pense que ce livre est un bon endroit pour commencer.  Et si vous avez pris un cours de statistiques traditionnel, j'espère que ce livre aidera à réparer les dommages.



---

Allen B. Downey est professeur d'informatique 
au Franklin W. Olin College of Engineering à Needham, MA.
...
Si vous avez une suggestion ou une correction à apporter, veuillez envoyer un courriel à l'adresse suivante : {\tt downey@allendowney.com}.  Si je fais un changement sur la base de vos commentaires, je vous ajouterai à la liste des contributeurs (sauf si vous demandez à être omis).
\index{contributors}

Si vous incluez au moins une partie de la phrase dans laquelle l'erreur apparaît, cela me facilite la recherche. Les numéros de page et de section sont également corrects, mais pas aussi faciles à utiliser.
Merci !
...
\item Lisa Downey et June Downey ont lu un premier projet et ont apporté de nombreuses corrections et suggestions.

\item Steven Zhang a trouvé plusieurs erreurs.

\item Andy Pethan et Molly Farison ont aidé à déboguer certaines des solutions, et Molly a repéré plusieurs fautes de frappe.

\item Dr. Nikolas Akerblom sait à quel point un Hyracotherium est grand.

\item Alex Morrow a clarifié l'un des exemples de code.

\item Jonathan Street a commis une erreur en un rien de temps.

\item Un grand merci à Kevin Smith et Tim Arnold pour leur travail sur plasTeX, que j'ai utilisé pour convertir ce livre en DocBook.

\item George Caplan a envoyé plusieurs suggestions pour améliorer la clarté.

\item Julian Ceipek a trouvé une erreur et un certain nombre de fautes de frappe.

\item Stijn Debrouwere, Leo Marihart III, Jonathan Hammler et Kent Johnson ont trouvé des erreurs dans la première édition imprimée.

\item J\"{o}rg Beyer a trouvé des fautes de frappe dans le livre et a fait de nombreuses corrections dans les docstrings du code d'accompagnement.

\item Tommie Gannert a envoyé un fichier correctif avec un certain nombre de corrections.

\item Christoph Lendenmann a soumis plusieurs errata.

\item Michael Kearney m'a envoyé de nombreuses et excellentes suggestions.

\item Alex Birch a fait un certain nombre de suggestions utiles.

\item Lindsey Vanderlyn, Griffin Tschurwald et Ben Small ont lu une première version de ce livre et ont trouvé de nombreuses erreurs.

\item John Roth, Carol Willing et Carol Novitsky ont effectué des révisions techniques du livre.  Ils ont trouvé de nombreuses erreurs et ont fait de nombreuses suggestions utiles.

\item David Palmer a envoyé de nombreuses suggestions et corrections utiles.

\item Erik Kulyk a trouvé de nombreuses fautes de frappe.

\itemNir Soffer a envoyé plusieurs excellentes requêtes de pull pour le livre et le code de soutien.

\item Flothesof, utilisateur de GitHub, a envoyé un certain nombre de corrections.

\item Toshiaki Kurokawa, qui travaille à la traduction japonaise de ce livre, a envoyé de nombreuses corrections et suggestions utiles.

\item Benjamin White a suggéré un code de pandas plus idiomatique.

\item Takashi Sato a repéré une erreur de code.

% ENDCONTRIB

\end{itemize}

D'autres personnes qui ont trouvé des fautes de frappe et des erreurs similaires sont Andrew Heine,
G\'{a}bor Lipt\'{a}k,
Dan Kearney,
Alexander Gryzlov, 
Martin Veillette, 
...
\chapter{L'analyse exploratoire des données}
\label{intro}

La thèse de ce livre est que les données combinées à des méthodes pratiques peuvent répondre aux questions et guider les décisions dans un contexte d'incertitude.

À titre d'exemple, je présente une étude de cas motivée par une question que j'ai entendue lorsque ma femme et moi attendions notre premier enfant : les premiers bébés ont-ils tendance à arriver en retard ?
\index{first babies}

Si vous recherchez cette question sur Google, vous trouverez de nombreuses discussions.  Certains prétendent que c'est vrai, d'autres disent que c'est un mythe, et d'autres encore que c'est l'inverse : les premiers bébés arrivent tôt.

Dans beaucoup de ces discussions, les gens fournissent des données pour étayer leurs affirmations.  J'ai trouvé de nombreux exemples de ce genre :

\begin{quote}

"Mes deux amies qui ont récemment donné naissance à leur premier bébé, ont eu un retard de près de deux semaines avant d'entrer en travail ou d'être induites."

"Mon premier est arrivé deux semaines en retard et maintenant je pense que le second va sortir deux semaines plus tôt !!"

"Je ne pense pas que cela puisse être vrai car ma soeur a été la première de ma mère et elle était précoce, comme beaucoup de mes cousins".

\end{quote}

Ces rapports sont appelés "preuves anecdotiques" car ils sont basés sur des données non publiées et généralement personnelles.  Dans les discussions informelles, les anecdotes n'ont rien de mal, je ne veux donc pas m'en prendre aux personnes que j'ai citées.
\index{anecdotal evidence}

Mais nous pourrions vouloir des preuves plus convaincantes et une réponse plus fiable.  Selon ces normes, les preuves anecdotiques échouent généralement, pour cause:

\begin{itemize}

\item Petit nombre d'observations: Si la durée de la grossesse est plus longue pour les premiers bébés, la différence est probablement faible par rapport à la variation naturelle.  Dans ce cas, il se peut que nous devions comparer un grand nombre de grossesses pour être sûrs qu'il existe une différence.
\index{pregnancy length}

\item Biais de sélection: Les personnes qui se joignent à une discussion sur cette question pourraient être intéressées parce que leurs premiers bébés étaient en retard.  Dans ce cas, le processus de sélection des données fausserait les résultats.
\index{selection bias}
\index{bias!selection}

\item Biais de confirmation:  Les personnes qui croient en l'affirmation pourraient être plus susceptibles de fournir des exemples qui la confirment.  Les personnes qui doutent de l'affirmation sont plus susceptibles de citer 
contre-exemples.
\index{confirmation bias}
\index{bias!confirmation}

\item L'inexactitude : Les anecdotes sont souvent des histoires personnelles, et souvent mal mémorisées, déformées, répétées de manière inexacte, etc.
...
\section{Une approche statistique}

Pour pallier les limites des anecdotes, nous utiliserons les outils des statistiques, qui comprennent :

\begin{itemize}

\item Collecte de données : Nous utiliserons des données provenant d'une grande enquête nationale qui a été conçue explicitement dans le but de générer des inférences statistiquement valables sur la population américaine.
\index{data collection}

\item Les statistiques descriptives : Nous produirons des statistiques qui résument les données de manière concise et évaluerons les différentes façons de visualiser les données.
\index{descriptive statistics}

\item L'analyse exploratoire des données : Nous rechercherons des modèles, des différences et d'autres caractéristiques qui répondent aux questions qui nous intéressent.  Dans le même temps, nous vérifierons les incohérences et identifierons les limites.
\index{exploratory data analysis}

\item L'estimation : Nous utiliserons les données d'un échantillon pour estimer les caractéristiques de la population générale.
\index{estimation}

\item Les tests d'hypothèses : Lorsque nous constatons des effets apparents, comme une différence entre deux groupes, nous évaluons si l'effet a pu se produire par hasard.
\index{hypothesis testing}

\end{itemize}

En appliquant ces étapes avec soin pour éviter les pièges, nous pouvons parvenir à des conclusions plus justifiables et plus susceptibles d'être correctes.


\section{L'enquête nationale sur la croissance des familles}
\label{nsfg}

Depuis 1973, les Centres américains de contrôle et de prévention des maladies (CDC) mènent l'enquête nationale sur la croissance des familles (NSFG), qui vise à recueillir "des informations sur la vie familiale, le mariage et le divorce, la grossesse, l'infertilité, l'utilisation de la contraception et la santé des hommes et des femmes". Les résultats de l'enquête sont utilisés pour planifier les services de santé et les programmes d'éducation à la santé, et pour réaliser des études statistiques sur les familles, la fécondité et la santé."  See
  \url{http://cdc.gov/nchs/nsfg.htm}.
\index{National Survey of Family Growth}
\index{NSFG}

Nous utiliserons les données recueillies par cette enquête pour déterminer si les premiers bébés ont tendance à arriver en retard, et pour répondre à d'autres questions.  Pour utiliser ces données efficacement, nous devons comprendre la conception de l'étude.

L'enquête NSFG est une étude {\bf "transversale"}, ce qui signifie qu'elle donne un aperçu d'un groupe à un moment donné.  L'alternative la plus courante est l'étude {\bf longitudinale}, qui observe un groupe de manière répétée sur une période de temps.
\index{cross-sectional study}
\index{study!cross-sectional}
\index{longitudinal study}
\index{study!longitudinal}

Le NSFG a été mené à sept reprises ; chaque déploiement est appelé {\bf cycle}. Nous utiliserons les données du cycle 6, qui a été mené de janvier 2002 à mars 2003.  \index{cycle}

L'objectif de l'enquête est de dégager des conclusions sur un {\bf population} ; la population cible de la NSFG est constituée par les personnes âgées de 15 à 44 ans aux États-Unis.  Idéalement, les enquêtes devraient recueillir des données auprès de chaque membre de la population, mais c'est rarement possible.  Nous recueillons plutôt des données auprès d'un sous-ensemble de la population appelé {\bf échantillon }.
Les personnes qui participent à une enquête sont appelées {\bf répondants }.
\index{population}

En général, les études transversales sont censées être {\bf représentatives}, ce qui signifie que chaque membre de la population cible a une chance égale de participer.  Cet idéal est difficile à atteindre dans la pratique, mais les personnes qui mènent les enquêtes s'en rapprochent le plus possible.
\index{respondent} \index{representative}

Le NSFG n'est pas représentatif, mais il est délibérément sur-échantillonné.  Les concepteurs de l'étude ont recruté trois groupes--Hispaniques, Afro-Américains et adolescents--à des taux supérieurs à leur représentation dans la population américaine, afin de s'assurer que le nombre de répondants dans chacun de ces groupes est suffisamment important pour tirer des conclusions statistiques valables.
\index{oversampling}

Bien sûr, l'inconvénient du suréchantillonnage est qu'il n'est pas aussi facile de tirer des conclusions sur la population générale à partir des statistiques de l'enquête.  Nous reviendrons sur ce point plus tard.

Lorsque l'on travaille avec ce type de données, il est important de se familiariser avec le {\bf codebook}, qui documente la conception de l'étude, les questions de l'enquête et l'encodage des réponses.  Le codebook et le guide de l'utilisateur pour les données NSFG sont disponibles à l'adresse suivante
\url{http://www.cdc.gov/nchs/nsfg/nsfg_cycle6.htm}


\section{L'importation des données}

Le code et les données utilisés dans ce livre sont disponibles à l'adresse suivante
\url{https://github.com/AllenDowney/ThinkStats2}.  Pour plus d'informations sur le téléchargement et l'utilisation de ce code, voir la section ~\ref{code}.

Une fois le code téléchargé, vous devez disposer d'un fichier appelé {\tt ThinkStats2/code/nsfg.py}.  Si vous l'exécutez, il devrait lire une donnée et imprimer un message du type "Tous les tests ont été réussis".

Voyons ce qu'il fait.  Les données de grossesse du cycle 6 de la NSFG sont dans un fichier appelé {\tt 2002FemPreg.dat.gz} ; il est un fichier de données compressé au format gzip en texte clair (ASCII), à largeur fixe colonnes.  Chaque ligne du fichier est un {\bf record} qui contient des données sur une grossesse.

Le format du fichier est documenté dans le fichier {\tt 2002FemPreg.dct}, qui est un fichier de dictionnaire Stata.  Stata est un logiciel de statistiques ;  un "dictionnaire" dans ce contexte est une liste de noms de variables, de types, et des indices qui identifient où  trouver chaque variable dans chaque ligne.

Par exemple, voici quelques lignes de {\tt 2002FemPreg.dct} :
%
\begin{verbatim}
infile dictionary {
...
Ce dictionnaire décrit deux variables : {\tt caseid} est une chaîne de 12 caractères qui représente l'ID de la répondante ; {\tt pregordr} est un entier d'un octet qui indique quelle grossesse cet enregistrement décrit pour cette répondante.

Le code que vous avez téléchargé comprend {\tt thinkstats2.py}, qui est un module Python qui contient de nombreuses classes et fonctions utilisées dans ce livre, y compris des fonctions qui lisent le dictionnaire Stata et le fichier de données NSFG.  Voici comment ils sont utilisés dans le fichier {\tt nsfg.py} :
...
{\tt ReadStataDct} prend le nom du fichier du dictionnaire
et renvoie {\tt dct}, un objet {\tt FixedWidthVariables} qui contient les informations du fichier du dictionnaire.  {\tt dct} fournit {\tt ReadFixedWidth}, qui lit le fichier de données.


\section{Les DataFrames}
\label{dataframe}

Le résultat de {\tt ReadFixedWidth} est un DataFrame, qui est la structure de données fondamentale fournie par les pandas, qui est un paquet de Pythondata et de statistiques que nous utiliserons tout au long de ce livre.
Un DataFrame contient une ligne pour chaque enregistrement, dans ce cas une ligne par grossesse, et une colonne pour chaque variable.
\index{pandas}
\index{DataFrame}

En plus des données, un DataFrame contient également les noms des variables et leurs types, et il fournit des méthodes pour accéder aux données et les modifier.

Si vous imprimez {\tt df}, vous obtenez une vue tronquée des lignes et des colonnes, et la forme du DataFrame, qui est de 13593 lignes/enregistrements et de 244 colonnes/variables.
...
Le DataFrame est trop grand pour être affiché, donc la sortie est tronquée.  La dernière ligne indique le nombre de lignes et de colonnes.

L'attribut {\tt columns} renvoie une séquence de noms de colonnes sous forme de chaînes Unicode :
...
Le résultat est un indice, qui est une autre structure de données de pandas.  
Nous en apprendrons plus sur Index plus tard, mais pour l'instant nous le traiterons comme une liste :
\index{pandas}
\index{Index}
...
Pour accéder à une colonne à partir d'un DataFrame, vous pouvez utiliser le nom de la colonne comme clé :
\index{DataFrame}
...
Le résultat est une série, encore une autre structure de données de pandas.
Une série est comme une liste en Python avec quelques fonctionnalités supplémentaires.
Lorsque vous imprimez une Series, vous obtenez les indices et les valeurs correspondantes :
\index{Series}
...
Dans cet exemple, les indices sont des entiers de 0 à 13592, mais en général, ils peuvent être de n'importe quel type triable.  Les éléments sont également des entiers, mais ils peuvent être de n'importe quel type.

La dernière ligne comprend le nom de la variable, la longueur de la série et le type de données ; {\tt int64} est l'un des types fournis par NumPy.  Si vous exécutez cet exemple sur une machine 32 bits, vous verrez peut-être {\tt int32}.
\index{NumPy}

Vous pouvez accéder aux éléments d'une série en utilisant des indices entiers et des tranches :
...
Le résultat de l'opérateur de l'indice est un {\tt int64} ; le résultat de la tranche est une autre série.

Vous pouvez également accéder aux colonnes d'un DataFrame en utilisant la notation par points :
\index{DataFrame}
...
Cette notation ne fonctionne que si le nom de la colonne est un identifiant Python valide, donc il doit commencer par une lettre, ne peut pas contenir d'espaces, etc.


\section{Variables}

Nous avons déjà vu deux variables dans l'ensemble de données NSFG, {\tt caseid} et {\tt pregordr}, et nous avons vu qu'il y a 244 variables au total. Pour les explorations de ce livre, j'utilise les variables suivantes :

\begin{itemize}

\item {\tt caseid} est l'identifiant entier du défendeur.

\item {\tt prglngth} La durée entière de la grossesse en semaines.
\index{pregnancy length} (durée de la grossesse)

\item {\tt outcome} est un code entier pour l'issue de la grossesse.  Le code 1 indique une naissance vivante.

\item {\tt pregordr} est un numéro d'ordre de grossesse ; par exemple, le code pour la première grossesse d'une répondante est 1, pour la deuxième grossesse est 2, et ainsi de suite.
\item {\tt birthord} est un numéro de série pour les naissances vivantes ; le code pour le premier enfant de l'enquêtée est 1, et ainsi de suite. Pour les résultats autres que les naissances vivantes, ce champ est vide.
\item \verb "birthwgt_lb" et \verb "birthwgt_oz" contiennent les parties en livres et en onces du poids du bébé à la naissance.
\index{birth weight} (poids à la naissance)
\index{weight!birth}

\item {\tt agepreg} est l'âge de la mère à la fin de la grossesse.

\item {\tt finalwgt} Le poids statistique associé à la personne interrogée.  Il s'agit d'une valeur à virgule flottante qui indique le nombre de la population américaine que ce répondant représente.
  \index{weight!sample}

\end{itemize}

Si vous lisez attentivement le livre de codes, vous constaterez que de nombreuses variables sont des {\bf recodes}, ce qui signifie qu'elles ne font pas partie des {\bf raw data} collectées par l'enquête ; elles sont calculées à partir des données brutes.  \index{recode} \index{raw data}

Par exemple, {\tt prglngth} pour les naissances vivantes est égal à la variable brute {\tt wksgest}. (semaines de gestation) si elle est disponible ; sinon, elle est estimée à l'aide de {\tt mostest * 4,33}. (le nombre de mois de gestation correspond au nombre moyen de semaines dans un mois).

Les recodages sont souvent basés sur une logique qui vérifie la cohérence et l'exactitude des données.  En général, il est bon d'utiliser les recodages lorsqu'ils sont disponibles, à moins qu'il n'y ait une raison impérieuse de traiter les données brutes vous-même.


\section{La transformation}
\label{cleaning}

Lorsque vous importez des données de ce type, vous devez souvent vérifier les erreurs, traiter des valeurs spéciales, convertir les données en différents formats et effectuer des calculs.  Ces opérations sont appelées {\bf nettoyage des données}.

{\tt nsfg.py} comprend {\tt CleanFemPreg}, une fonction qui nettoie les variables que je prévois d'utiliser.
...
{\tt agepreg} contient l'âge de la mère à la fin de la
grossesse.  Dans le fichier de données, {\tt agepreg} est encodé comme un nombre entier de centimètres.  Ainsi, la première ligne divise chaque élément de {\tt agepreg} par 100, ce qui donne une valeur en virgule flottante
ans.

\verb "birthwgt_lb" et \verb "birthwgt_oz" contiennent le poids du bébé, en livres et en onces, pour les grossesses qui se terminent par une naissance vivante. En outre, il utilise plusieurs codes spéciaux :
...
Les valeurs spéciales encodées sous forme de nombres sont dangereuses car si elles ne sont pas manipulées correctement, elles peuvent générer des résultats bidons, comme un bébé de 90 kilos.  La méthode de remplacement remplace ces valeurs par np.nan, une valeur spéciale à virgule flottante qui représente "pas un nombre".  L'indicateur {\tt inplace} indique à la méthode {\tt replace} de modifier la série existante plutôt que d'en créer une nouvelle.
\index{NaN}

Dans le cadre de la norme en virgule flottante de l'IEEE, toutes les opérations mathématiques renvoient {\tt nan} si l'un ou l'autre des arguments est {\tt nan} :
...
Ainsi, les calculs effectués avec les {\tt nan} tendent à être corrects, et la plupart des fonctions pandas gèrent les {\tt nan} de manière appropriée.  Mais le traitement des données manquantes sera un problème récurrent.
\index{pandas}
\index{missing values}

La dernière ligne de {\tt CleanFemPreg} crée une nouvelle colonne \verb "totalwgt_lb" qui combine les livres et les onces en une seule quantité, en livres.

Une remarque importante : lorsque vous ajoutez une nouvelle colonne à un DataFrame, vous devez utiliser la syntaxe du dictionnaire, comme ceci
\index{DataFrame}
...
La version avec notation par points ajoute un attribut à l'objet DataFrame, mais cet attribut n'est pas traité comme une nouvelle colonne.


\section{La validation}

Lorsque des données sont exportées d'un environnement logiciel et importées dans un autre, des erreurs peuvent être introduites.  Et lorsque vous vous familiarisez avec un nouvel ensemble de données, vous pouvez interpréter les données de manière incorrecte ou introduire d'autres malentendus.  Si vous prenez le temps de valider les données, vous pouvez gagner du temps plus tard et éviter les erreurs.

Une façon de valider les données est de calculer des statistiques de base et de les comparer aux résultats publiés.  Par exemple, le codebook de NSFG comprend des tableaux qui résument chaque variable.  Voici le tableau pour les {\tt outcome} de la grossesse, qui codifie le résultat de chaque grossesse :
...
La classe Series fournit une méthode, \verb "value_counts", qui compte le nombre de fois que chaque valeur apparaît.  Si nous sélectionnons le {\tt outcome}  Série de la DataFrame, nous pouvons utiliser \verb "value_counts" pour comparer avec les données publiées :
\index{DataFrame}
\index{Series}
...
Le résultat de \verb "value_counts" est une série ; \verb "sort_index()" trie la série par index, de sorte que les valeurs apparaissent dans l'ordre.

En comparant les résultats avec le tableau publié, il semble que les valeurs dans {\tt outcome} soient correctes.  De même, voici le tableau publié pour \verb "birthwgt_lb"
...
Les chiffres de 6, 7 et 8 livres sont valables, et si vous additionnez les chiffres de 0 à 5 et de 9 à 95, ils sont également valables.  Mais si vous regardez de plus près, vous remarquerez une valeur qui doit être une erreur, un bébé de 51 livres !

Pour remédier à cette erreur, j'ai ajouté une ligne dans le fichier {\tt CleanFemPreg} :

\begin{verbatim}
df.loc[df.birthwgt_lb > 20, 'birthwgt_lb'] = np.nan
\end{verbatim}

Cette déclaration remplace les valeurs non valides par {\tt np.nan}.
L'attribut {\tt loc} offre plusieurs façons de sélectionner des lignes et des colonnes dans un DataFrame.  Dans cet exemple, la première expression entre parenthèses est l'indexeur de ligne ; la deuxième expression sélectionne la colonne.
\index{loc indexer}
\index{indexer!loc}

L'expression \verb "df.birthwgt_lb > 20" donne une série de type {\tt bool}, où True indique que la condition est vraie.  Lorsqu'une série booléenne est utilisée comme indice, elle ne sélectionne que les éléments qui satisfont à la condition.
\index{Series} \index{boolean} \index{NaN}



\section{L'interprétation}

Pour travailler efficacement avec les données, il faut penser à deux niveaux en même temps : le niveau des statistiques et le niveau du contexte.

À titre d'exemple, examinons la séquence des résultats pour quelques répondants.  En raison de la façon dont les fichiers de données sont organisés, nous devons effectuer un certain traitement pour recueillir les données sur la grossesse de chaque répondante. Voici une fonction qui fait cela :
...
{\tt df} est le DataFrame avec les données sur la grossesse.  La méthode {\tt iteritems} énumère l'index (numéro de ligne) et la {\tt caseid} pour chaque grossesse.
\index{DataFrame}

{\tt d}est un dictionnaire qui établit une correspondance entre chaque identification de cas et une liste de
indices.  Si vous ne connaissez pas {\tt defaultdict}, c'est dans
le module Python {\tt collections}.
En utilisant {\tt d}, nous pouvons rechercher une répondante et obtenir les indices des grossesses de cette répondante.

Cet exemple permet de rechercher une personne interrogée et d'imprimer une liste des résultats de ses grossesses :
...
{\tt indices} est la liste des indices de grossesses correspondant à la répondante {\tt 10229}.

L'utilisation de cette liste comme index dans le fichier {\tt df.outcome} permet de sélectionner les lignes indiquées et d'obtenir une série.  Au lieu d'imprimer toute la série, j'ai sélectionné l'attribut {\tt values}, qui est un tableau NumPy.  
\index{NumPy}
\index{Series}

Le code de résultat {\tt 1} indique une naissance vivante. Le code {\tt 4} indique une fausse-couche, c'est-à-dire une grossesse qui s'est terminée spontanément, généralement sans cause médicale connue.

Statistiquement, cette répondante n'est pas inhabituelle.  Les fausses couches sont courantes et il y a d'autres répondants qui en ont déclaré autant ou plus.

Mais si l'on se souvient du contexte, ces données racontent l'histoire d'une femme qui a été enceinte six fois, se terminant chaque fois par une fausse couche.  Sa septième grossesse, la plus récente, s'est terminée par une naissance vivante.  Si nous considérons ces données avec empathie, il est naturel d'être ému par l'histoire qu'elles racontent.

Chaque enregistrement de l'ensemble de données NSFG représente une personne qui a fourni des réponses honnêtes à de nombreuses questions personnelles et difficiles.  Nous pouvons utiliser ces données pour répondre à des questions statistiques sur la vie familiale, la reproduction et la santé.  En même temps, nous avons l'obligation de prendre en considération les personnes représentées par les données, et de leur accorder respect et gratitude.
\index{ethics}
